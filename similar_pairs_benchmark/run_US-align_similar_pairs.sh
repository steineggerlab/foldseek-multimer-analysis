...
while read line || [ -n "$line" ] ; do
  ...
  n=${array[2]}
  mkdir tmp
  /usr/bin/time $1 ../datasets/similar_pairs_benchmark/PDBs/"$q".pdb  ../datasets/similar_pairs_benchmark/PDBs/"$t".pdb  -mm 1 -ter 0 -mol prot -outfmt 2 > tmp/result.tsv 2> tmp/log.txt
