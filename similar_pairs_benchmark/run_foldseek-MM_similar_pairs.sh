...
while read line || [ -n "$line" ] ; do
  ...
  n=${array[2]}
  mkdir tmp
  /usr/bin/time $1 easy-complexsearch ../datasets/similar_pairs_benchmark/PDBs/"$q".pdb ../datasets/similar_pairs_benchmark/PDBs/"$t".pdb tmp/result tmp --threads 1 --exhaustive-search 1 -v 0 2> tmp/log.txt
