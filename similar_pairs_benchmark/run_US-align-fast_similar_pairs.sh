...
while read line || [ -n "$line" ] ; do
  ...
  n=${array[2]}
  mkdir $DIR/tmp
  /usr/bin/time $1 $DIR/../datasets/similar_pairs_benchmark/PDBs/"$q".pdb  $DIR/../datasets/similar_pairs_benchmark/PDBs/"$t".pdb  -mm 1 -ter 0 -mol prot -outfmt 2 -fast > $DIR/tmp/result.tsv 2> $DIR/tmp/log.txt
